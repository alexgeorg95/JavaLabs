...
public void run() {
	...
	try {
		...
		while((str = is.readLine())!=null) {
			if(str.equals("exit")) break; // прекратить передачу сообщений при вводе "exit" и покинуть чат
			for(ClientsConnected c: clients){
			if (c.name.equals(recipient)){
			c.send("Message from @" + name + ":" + str); // отправить сообщение выбранному из списка клиенту
				}
			}
		}
		/*for (ClientsConnected c :clients){
